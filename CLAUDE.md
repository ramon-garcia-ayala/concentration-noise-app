...
- **audioBus.js** — Central audio routing. All Web Audio nodes connect through a shared `GainNode → AnalyserNode → destination` chain. Exports: channel volume tracking, massage mode (interval-based LFO), mute toggle, mascot state (`getMascotState`/`setMascotState` — shared global so any component can read the current cat animation state). Master volume defaults to 50%
- **Components:**
  ...
  - `PixelMascot` — Kawaii pixel cat (10x12 grid). Token-based sprite system (F/Fd/Fw/Fp/E/Ep). 8 named cat color presets (persisted to localStorage as `cat-config`). Reacts to audio levels and updates `setMascotState()` each frame. Dynamic animated tail drawn per-frame (not baked into sprites). Click reactions: wink (sleeping, curled body), love with hearts + head dip (awake, eyes closed). Sends `mascot-update` IPC at ~10fps for widget sync. Inline color selector below cat (dress button swaps with color circles). Only Zzz particles when sleeping, no particles when awake
  - `Widget` — Floating overlay (`src/renderer/src/components/Widget.jsx`). Duplicates all cat sprites and motion functions from PixelMascot (must be kept in sync manually). Has animated SVG progress border (inside panel div, not outer container). Play/pause via `toggleTimer` IPC. Circular volume knob controls app master volume (scroll or drag), synced bidirectionally via `set-master-volume` IPC. Cat clicks relay to main app via `mascot-click` IPC for synchronized reactions. Cat preset and animation state synced in real-time via `mascot-update` IPC (~10fps). Interactive elements use `WebkitAppRegion: 'no-drag'` to override parent drag region
...
- `timer-update` — PomodoroTimer → main → Widget (timer state, progress, mascot, masterVolume, catPreset)
- `mascot-update` — PixelMascot → main → Widget (mascot state + catPreset at ~10fps for responsive sync)
- `mascot-click` — Widget → main → PixelMascot (click reaction sync)
